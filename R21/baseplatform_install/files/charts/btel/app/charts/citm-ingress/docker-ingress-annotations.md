...
!!! note
    ...
    [`--annotations-prefix` command line argument](docker-ingress-cli-arguments.md),
...
CITM specific annotations

|Name                       | type |
|---------------------------|------|
|[nginx.ingress.kubernetes.io/http2-plain-text](#http2-plain-text)|bool|
|[nginx.ingress.kubernetes.io/proxy-pass-suffix](#proxy-pass-suffix)|string|
|[ingress.citm.nokia.com/sticky-route-services](#sticky-route-services)|string. This is a deprecated annotation. Considere using (#session-affinity)|
...
!!! example
    Please check the [affinity](ingress-cookie.md) example.
...
This is similar to [load-balance](docker-ingress-configmap.md#load-balance) but configures load balancing algorithm per ingress.
...
The ingress controller requires a [default backend](docker-default404.md).
...
!!! note
    ...
    [`--enable-ssl-passthrough`](docker-ingress-cli-arguments.md) flag.
...
This annotation will be deprecated. Please use `nginx.ingress.kubernetes.io/backend-protocol: "HTTPS"`
...
[configmap]: ./docker-ingress-configmap.md
...
[ConfigMap](./docker-ingress-configmap.md#enable-modsecurity). Note this will enable ModSecurity for all paths, and each path
...
Use this annotation if you want to activate http2 on http plain text (h2c) for this ingress.

See also controller.enableHttp2OnHttp https://gitlabe1.ext.net.nokia.com/csf_bcmt/gerrit/CSF-CHARTS/raw/1.0/citm-ingress/README.md#configuration, for activating it on all ingresses

### proxy-pass-suffix

Use this annotation if you want to extends proxy_pass directive. By default, proxy_pass is set to the stream backend associated to your ingress
```yaml
  proxy_pass https://sticky-cookie-default-tomcat-8080
```
In case, you have to adapt the proxy pass suffix, provide the content that needs to be added. 
```yaml
  nginx.ingress.kubernetes.io/proxy-pass-suffix: $request_uri
```

Will generate a proxy_pass directive like

```yaml
  proxy_pass https://sticky-cookie-default-tomcat-8080
```

### sticky-route-services

This is a deprecated annotation. Considere using [session-affinity](#session-affinity) or [cookie-affinity](#cookie-affinity)
