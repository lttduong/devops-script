...
| timezone.mountHostLocaltime | false | ==NEW IN 7.13.4== Should /etc/localtime from kubernetes host be mounted in all statefulset containers? | [Yes^1^](#"common_fn1") |
| timezone.timeZoneEnv | *(omitted)* | ==NEW IN 7.13.4== Set as TZ environment variable in all statefulset containers. | [Yes^1^](#"common_fn1") |
...
| services.mariadb.name | <Chart Release\>-mariadb-<N> | ==NEW IN 7.13.4== If Istio is enabled, the name of the Kubernetes Service for the Istio per-pod service. | No |
| services.mariadb.exporter.name | <Chart Release\>-mariadb-metrics | ==NEW IN 7.13.4== If mariadb.metrics.enabled is set to true, this overrides the Kubernetes Service name be used for metrics collection. | No |
| services.mariadb.exporter.port | 9104 | ==CHANGED IN 7.13.4== If mariadb.metrics.enabled is set to true, this port can be configured to define the port which mysqld\_exporter will listen to for metrics collection.<br>**NOTE: Renamed from services.mariadb.exporter\_port** | Yes |
...
| services.maxscale.exporter.name | <Chart Release\>-maxscale-metrics | ==NEW IN 7.13.4== If maxscale.metrics.enabled is set to true, this overrides the Kubernetes Service name be used for metrics collection. | No |
| services.maxscale.exporter.port | 9195 | ==CHANGED IN 7.13.4== If maxscale.metrics.enabled is set to true, this port can be configured to define the port which maxscale\_exporter will listen to for metrics collection.<br>**NOTE: Renamed from services.maxscale.exporter\_port** | Yes |
...
| services.endpoints.master.name | <Chart Release\>-master-<geo_redundancy.remote.name\> | ==NEW IN 7.13.4== The name of the Kubernetes Service providing a service endpoint for the remote data center connection to local Master service. *(Only relevant in Master-Slave clusters with MaxScale)* | No |
| services.endpoints.maxscale.name | <Chart Release\>-maxscale-<geo_redundancy.remote.name\> | ==NEW IN 7.13.4== The name of the Kubernetes Service providing a service endpoint for the remote data center connection to local Maxscale service. *(Only relevant in Master-Slave clusters with MaxScale)* | No |
...
| mariadb.mysqldOpts | *(omitted)* | ==NEW IN 7.13.4== Additional command line arguments to feed to mysqld (mariadb) process.  See [mysqld Options](https://mariadb.com/kb/en/mysqld-options/) for full list of mysqld options. | [Yes^1^](#mariadb_fn1) |
