...
readonly NIAM_INDICES="commands_niam\|session\|videologsearch"
readonly SNAPSHOT_INDICES=$(cat $SNAPSHOT_INDICES_PATH 2>/dev/null | grep $NIAM_INDICES | tr -d " ," | tr -d '"')
readonly SNAPSHOT_INDICES_WITH_FORMATTING=$(cat $SNAPSHOT_INDICES_PATH 2>/dev/null | grep $NIAM_INDICES | tr -d " ," | sed -z "s/\n\"/,/g" | tr -d '"')
...
readonly CURL_TEMPLATE="curl --insecure --silent --cert ${ADMIN_CERTIFICATE} --key ${ADMIN_KEY} \
    -X %s https://$ELASTICSEARCH_SERVICE:$CLIENT_PORT/%s"
readonly SUCCESS=0
...
function should_be_deleted_if_index_has_suffix() (
    ...
    index=$2
    index_prefix=$3
    [[ ${index_from_snapshot} == ${index_prefix}${INDEX_SUFFIX} && ${index} == ${index_prefix}${INDEX_SUFFIX} ]] && \
        return 0 || return 1
)
...
function should_be_deleted_if_index_has_not_suffix() (
    ...
    index=$2
    index_prefix=$3
    [[ ${index_from_snapshot} != ${index_prefix}${INDEX_SUFFIX} && ${index} == "${index_prefix}" ]] && \
        return 0 || return 1
)
        ...
        for index in ${current_indices[*]}; do
              if should_be_deleted_if_index_has_suffix "${index_from_snapshot}" "${index}" "${index_prefix}" ||
                 should_be_deleted_if_index_has_not_suffix "${index_from_snapshot}" "${index}" "${index_prefix}"; then
                  if ! echo "${indices_to_delete[*]}" | grep -q -w "${index}"; then
                      indices_to_delete+=("${index}") && delete_index "${index}"
                  fi
              fi
        done
    done
...
function restore_niam_snapshot() (
    if [ -n "$SNAPSHOT_INDICES" ]; then
        log_info "Restore of Elasticsearch NIAM data has started"
        restore_output=$(mktemp)
        printf -v restore_niam_snapshot_request "${CURL_TEMPLATE}" \
            POST _snapshot/es_backup/"${SNAPSHOT_NAME}"/_restore?wait_for_completion=true
        log_info_ne "Snapshot restore in progress"
        ${restore_niam_snapshot_request} \
            -H 'Content-Type: application/json' \
            -d '{ "indices": "'"$SNAPSHOT_INDICES_WITH_FORMATTING"'", "ignore_unavailable": true,"include_global_state": true }' > ${restore_output} &
        pid=$(echo $!)
        ps $pid > /dev/null
    fi
    if [ -z "$SNAPSHOT_INDICES" ]; then
        log_info "No NIAM indices to restore"
        exit $SUCCESS
    fi
    while [[ $? == $SUCCESS ]]; do
        echo -ne "."
...
restore_niam_snapshot

log_info "Post restore of Elasticsearch NIAM data completed"
